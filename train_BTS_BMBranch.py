...
from model.trainer import XMemTrainer, XMemTrainer_swin_multi
from dataset.static_dataset import StaticTransformDataset
from dataset.vos_dataset import VOSDataset, VOSDataset_multi

from util.logger import TensorboardLogger
from util.configuration import Configuration, Configuration_swin_multi
...
raw_config = Configuration_swin_multi()
raw_config.parse()
...
for si, stage in enumerate(stages_to_perform):
    ...
    if local_rank == 0:
        ...
        model = XMemTrainer_swin_multi(config, logger=logger, 
        ...
        model = XMemTrainer_swin_multi(config, local_rank=local_rank, world_size=world_size).train()
    ...
    def renew_vos_loader_multi(max_skip, max_num_obj=1, finetune=False):
        ceus_dataset = VOSDataset_multi(path.join(ceus_root, 'BM_JPEGImages'), path.join(ceus_root, 'BM_JPEGImages'), 
                                        path.join(ceus_root, 'Annotations'), max_skip, is_bl=False, subset=None, num_frames=config['num_frames'], max_num_obj=max_num_obj, finetune=finetune)

        return construct_loader(ceus_dataset)
    ...
    if stage == '0':
        ...
        ceus_root = path.join(path.expanduser(config['ceus_root']), 'train_ROI')

        train_sampler, train_loader = renew_vos_loader_multi(max_skip=5, max_num_obj=3)
        renew_loader = renew_vos_loader_multi
    ...
    try:
        with tqdm(total=config['iterations'] + config['finetune'], desc="Training Progress") as pbar:
