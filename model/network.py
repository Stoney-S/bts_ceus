...
# Added for CEUS
class XMem_swin_multi(nn.Module):
    def __init__(self, config, model_path=None, map_location=None):
        """
        model_path/map_location are used in evaluation only
        map_location is for converting models saved in cuda to cpu
        ...
        self.single_object = config.get('single_object', True) # False -> True
        print(f'Single object mode: {self.single_object}')

        self.key_encoder = KeyEncoder_swin_multi()
        self.value_encoder = ValueEncoder_multi(self.value_dim, self.hidden_dim, self.single_object)
    ...
    def encode_key(self, ce_frame, bm_frame, need_sk=True, need_ek=True):
        # Determine input shape
        if len(ce_frame.shape) == 5 and len(bm_frame.shape) == 5:
            # shape is b*t*c*h*w
            need_reshape = True
            b, t = ce_frame.shape[:2]
            # flatten so that we can feed them into a 2D CNN
            ce_frame = ce_frame.flatten(start_dim=0, end_dim=1) # B*T, C, H, W
            bm_frame = bm_frame.flatten(start_dim=0, end_dim=1) # B*T, C, H, W
        elif len(ce_frame.shape) == 4 and len(bm_frame.shape) == 4:
            # shape is b*c*h*w
            need_reshape = False
        else:
            raise NotImplementedError
    
        f16, f8, f4 = self.key_encoder(ce_frame, bm_frame)
        key, shrinkage, selection = self.key_proj(f16, need_sk, need_ek)
    ...
    def encode_value(self, ce_frame, bm_frame, image_feat_f16, h16, masks, is_deep_update=True):
        # masks: B, C, H, W
        num_objects = masks.shape[1]
        if num_objects != 1:
            others = torch.cat([
        ...
        # Pass both ce_frame and bm_frame to value_encoder
        g16, h16 = self.value_encoder(ce_frame, bm_frame, image_feat_f16, h16, masks, others, is_deep_update)
    ...
    def init_hyperparameters(self, config, model_path=None, map_location=None):
        ...
        if model_path is not None:
            ...
            model_weights = model_weights['network'] # added
            # print('debugging')
            # for key, value in model_weights['network'].items():
