...
# Added for CEUS
class ValueEncoder_multi(nn.Module):
    def __init__(self, value_dim, hidden_dim, single_object=False):
        super().__init__()
    ...
    def forward(self, ce_image, bm_image, image_feat_f16, h, masks, others, is_deep_update=True):
        # ce_image, bm_image are of shape [B, C, H, W]
        # Convert ce_image and bm_image to grayscale by averaging the color channels (if they are RGB).
        ce_gray = ce_image.mean(dim=1, keepdim=True)  # B, C=1, H, W
        bm_gray = bm_image.mean(dim=1, keepdim=True)  # B, C=1, H, W
        avg_gray = (ce_gray + bm_gray) / 2            # B, C=1, H, W

        # Stack these grayscale images into an RGB format
        image = torch.cat([ce_gray, bm_gray, avg_gray], dim=1)  # B, C=3, H, W

        # image_feat_f16 is the feature from the key encoder
        if not self.single_object:
            g = torch.stack([masks, others], 2)
        else:
            g = masks.unsqueeze(2)  # B, N, 1, H, W
        g = self.distributor(image, g)  # B, N, C+1, H, W
        ...
        g = g.view(batch_size, num_objects, *g.shape[1:]) # g: 2, 1, 256, 24, 24. image_feat_f16: 2, 1024, 24, 24
        # print(f'g shape is {g.shape} before fusion')
        # print(f'image_feat_16 shape is {image_feat_f16.shape} before fusion')
        g = self.fuser(image_feat_f16, g) # 2, 1, 512, 24, 24
        # print(f'g shape is {g.shape} after fusion')
...
# Added for CEUS
class KeyEncoder_swin_multi(nn.Module):
    def __init__(self):
        super().__init__()
        
        self.swin_contrast = create_model("swin_base_patch4_window12_384", pretrained=False, features_only=True)
        self.swin_routine = create_model("swin_base_patch4_window12_384", pretrained=False, features_only=True)
        del self.swin_routine.layers_3
        del self.swin_contrast.layers_3
        
    def forward(self, contrast_frame, routine_frame):
        features_contrast = self.swin_contrast(contrast_frame)
        features_routine = self.swin_routine(routine_frame)
        
        # Check for expected feature layers to prevent out-of-bound indexing
        if len(features_contrast) >= 3 and len(features_routine) >= 3:
            # Concatenate the feature maps along the channel dimension
            f4 = torch.cat((features_contrast[0].permute(0, 3, 1, 2), 
                            features_routine[0].permute(0, 3, 1, 2)), dim=1)  # 1/4 scale
            f8 = torch.cat((features_contrast[1].permute(0, 3, 1, 2), 
                            features_routine[1].permute(0, 3, 1, 2)), dim=1)  # 1/8 scale
            f16 = torch.cat((features_contrast[2].permute(0, 3, 1, 2), 
                             features_routine[2].permute(0, 3, 1, 2)), dim=1) # 1/16 scale
        else:
            raise ValueError("The model did not return enough feature maps at different scales.")
