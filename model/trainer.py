...
from model.losses import LossComputer, LossComputer_multi
from util.log_integrator import Integrator
from util.image_saver import pool_pairs, pool_pairs_multi
...
class XMemTrainer_swin_multi:
    def __init__(self, config, logger=None, save_path=None, local_rank=0, world_size=1):
        ...
        if logger is not None:
            ...
            self.logger.log_string('model_size', str(sum([param.nelement() for param in self.XMem.parameters()])))
        self.train_integrator = Integrator(self.logger, distributed=True, local_rank=local_rank, world_size=world_size)
        self.loss_computer = LossComputer_multi(config)
    ...
    def do_pass(self, data, it=0):
        ...
        # Modifications: separate contrast and routine frames from the data dictionary
        ce_frames = data['contrast_enhanced']  # B, T, C, H, W
        bm_frames = data['bmode']              # B, T, C, H, W
        first_frame_gt = data['first_frame_gt'].float() # B, T, C, H, W; T=1, C=max_num_obj (which is 3 by default), only the 1st channel has annotation in our case

        b = ce_frames.shape[0]
        num_filled_objects = [o.item() for o in data['info']['num_objects']]
        num_objects = first_frame_gt.shape[2]
        selector = data['selector'].unsqueeze(2).unsqueeze(2) # B, C, 1, 1, it's for object channel

        with torch.cuda.amp.autocast(enabled=self.config['amp']):
            # Instead of encoding `frames`, we pass `ce_frames` and `bm_frames`
            key, shrinkage, selection, f16, f8, f4 = self.XMem('encode_key', ce_frames, bm_frames)

            filler_one = torch.zeros(1, dtype=torch.int64)
            hidden = torch.zeros((b, num_objects, self.config['hidden_dim'], *key.shape[-2:]))

            # Modifications: encode_value now uses both frames at t=0, [B, C, H, W]
            v16, hidden = self.XMem('encode_value', ce_frames[:, 0], bm_frames[:, 0], f16[:, 0], hidden, first_frame_gt[:, 0])
            values = v16.unsqueeze(3)  # add the time dimension
            ...
            for ti in range(1, self.num_frames):
                ...
                if ti < (self.num_frames-1):
                    is_deep_update = np.random.rand() < self.deep_update_prob
                    v16, hidden = self.XMem('encode_value', ce_frames[:, ti], bm_frames[:, ti], f16[:,ti], hidden, masks, is_deep_update=is_deep_update)
                    values = torch.cat([values, v16.unsqueeze(3)], 3)
                    ...
                    if self._is_train:
                        if it % self.log_image_interval == 0 and it != 0:
                            if self.logger is not None:
                                images = {**data, **out}
                                size = (384, 384)
                                self.logger.log_cv2('train/pairs', pool_pairs_multi(images, size, num_filled_objects), it) # pool_pairs -> pool_pairs_multi
