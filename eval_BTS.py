...
from inference.data.test_datasets import LongTestDataset, DAVISTestDataset, YouTubeVOSTestDataset, CEUSVOSTestDataset
from inference.data.mask_mapper import MaskMapper
from model.network import XMem, XMem_swin_multi
from inference.inference_core import InferenceCore, InferenceCore_multi
...
parser.add_argument('--ceus_model', default='saves/CEBranch_s4/xmem_v4.pth')
...
# added for ceus
parser.add_argument('--ceus_path', default='data')
# For generic (G) evaluation, point to a folder that contains "JPEGImages" and "Annotations"
parser.add_argument('--generic_path')

parser.add_argument('--dataset', help='D16/D17/Y18/Y19/LV1/LV3/G/CEUS', default='CEUS') # added for ceus

# Adjust here for different evaluation set
parser.add_argument('--split', help='val_ROI/internal_test/external_test_A/external_test_B', default='external_test_B')
...
if args.output is None:
    args.output = f'data/output/xmem_CEBranch/{args.dataset}_{args.split}'
...
is_ceus = args.dataset.startswith('CEUS')
...
if is_youtube:
    ...
    ceus_root = f'{args.ceus_path}/{args.split}'
    meta_dataset = CEUSVOSTestDataset(path.join(ceus_root, 'CE_JPEGImages'), path.join(ceus_root, 'CE_JPEGImages'), path.join(ceus_root, 'Annotations'), size=-1)
...
# Load our checkpoint
network = XMem_swin_multi(config, args.ceus_model).cuda().eval()
if args.model is not None:
    model_weights = torch.load(args.ceus_model)
...
# for vid_reader in bar(meta_loader):
for vid_reader in tqdm(meta_loader, total=len(meta_dataset), desc="Processing videos"):
    ...
    mapper = MaskMapper()
    processor = InferenceCore_multi(network, config=config)
    first_mask_loaded = False

    for ti, data in enumerate(loader):
        with torch.cuda.amp.autocast(enabled=not args.benchmark):
            ce_img = data['contrast_enhanced'].cuda()[0]
            bm_img = data['bmode'].cuda()[0]
            msk = data.get('mask')
            H, W, left, top, right, bottom = map(int, data['coords'])
            info = data['info']
            frame = info['frame'][0]
            shape = info['shape']
            ...
            flag = True
            if msk is not None:
                if msk.sum() == 0: # in case no object
                    flag = False
            ...
            if flag:
                ...
                if args.flip:
                    ce_img = torch.flip(ce_img, dims=[-1])
                    bm_img = torch.flip(bm_img, dims=[-1])
                ...
                prob = processor.step(ce_img, bm_img, msk, labels, end=(ti==vid_length-1))
                ...
                if args.save_all or info['save'][0]:
                    ...
                    background = Image.new("L", (H, W), 0)  # "L" mode for grayscale, 0 for black
                    paste_left = max(0, left)
                    paste_top = max(0, top)
                    paste_right = min(W, right)
                    paste_bottom = min(H, bottom)

                    crop_left = 0 if left >= 0 else abs(left)
                    crop_top = 0 if top >= 0 else abs(top)
                    crop_right = out_img.width - (right - W) if right > W else out_img.width
                    crop_bottom = out_img.height - (bottom - H) if bottom > H else out_img.height

                    out_img = out_img.crop((crop_left, crop_top, crop_right, crop_bottom))
                    background.paste(out_img, (paste_left, paste_top))

                    background.save(os.path.join(this_out_path, frame[:-4]+'.png'))
