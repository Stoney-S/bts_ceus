...
for si, stage in enumerate(stages_to_perform):
    ...
    def renew_vos_loader_multi(max_skip, max_num_obj=1, finetune=False):
        ceus_dataset = VOSDataset_multi(path.join(ceus_root, 'CE_JPEGImages'), path.join(ceus_root, 'BM_JPEGImages'), 
    ...
    if stage == '0':
        ...
        ceus_root = path.join(path.expanduser(config['ceus_root']), 'train')
