...
class VOSDataset_multi(Dataset):
    """
    Works for CEUS training
    For each sequence:
    - Pick three frames
    - Pick two objects
    - Apply some random transforms that are the same for all frames
    ...
    - The distance between frames is controlled
    """
    def __init__(self, ce_im_root, bm_im_root, gt_root, max_jump, is_bl, subset=None, num_frames=3, max_num_obj=3, finetune=False):
        # Added ce_im_root and bm_im_root as input parameters
        self.ce_im_root = ce_im_root  # Contrast-enhanced image root
        self.bm_im_root = bm_im_root  # B-mode image root
        self.gt_root = gt_root
        self.max_jump = max_jump  # Maximum frame jump
        self.is_bl = is_bl
        self.num_frames = num_frames
        self.max_num_obj = max_num_obj
        ...
        vid_list = sorted(os.listdir(self.ce_im_root))
        # Pre-filtering
        for vid in vid_list:
            if subset is not None:
                if vid not in subset:
                    continue
            frames = sorted(os.listdir(os.path.join(self.ce_im_root, vid)))
            if len(frames) < num_frames:
                continue
            self.frames[vid] = frames  # key: video name, value: [XX.jpg, ...]
            self.videos.append(vid)

        print('%d out of %d videos accepted in %s.' % (len(self.videos), len(vid_list), ce_im_root))
    ...
    def __getitem__(self, idx):
        video = self.videos[idx]  # folder name (video name without extension)
        info = {'name': video}

        ce_vid_im_path = path.join(self.ce_im_root, video)
        bm_vid_im_path = path.join(self.bm_im_root, video)
        vid_gt_path = path.join(self.gt_root, video)
        frames = self.frames[video]
        ...
        while trials < 5:
            ...
            sequence_seed = np.random.randint(2147483647)
            ce_images = []  # Contrast-enhanced images
            bm_images = []  # B-mode images
            masks = []
            target_objects = []
            for f_idx in frames_idx:
                jpg_name = frames[f_idx][:-4] + '.jpg'
                ...
                reseed(sequence_seed)
                ce_im = Image.open(path.join(ce_vid_im_path, jpg_name)).convert('RGB')
                ce_im = self.all_im_dual_transform(ce_im)
                ce_im = self.all_im_lone_transform(ce_im)
                
                reseed(sequence_seed)
                bm_im = Image.open(path.join(bm_vid_im_path, jpg_name)).convert('RGB')
                bm_im = self.all_im_dual_transform(bm_im)
                bm_im = self.all_im_lone_transform(bm_im)
                ...
                pairwise_seed = np.random.randint(2147483647)
                reseed(pairwise_seed)
                ce_im = self.pair_im_dual_transform(ce_im)
                ce_im = self.pair_im_lone_transform(ce_im)
                
                reseed(pairwise_seed)
                bm_im = self.pair_im_dual_transform(bm_im)
                bm_im = self.pair_im_lone_transform(bm_im)

                ce_im = self.final_im_transform(ce_im)
                bm_im = self.final_im_transform(bm_im)
                this_gt = np.array(this_gt)

                ce_images.append(ce_im)
                bm_images.append(bm_im)
                masks.append(this_gt)

            ce_images = torch.stack(ce_images, 0)
            bm_images = torch.stack(bm_images, 0)
