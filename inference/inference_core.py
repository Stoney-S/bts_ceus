...
class InferenceCore_multi:
    def __init__(self, network:XMem_swin_multi, config):
        self.config = config
        self.network = network
        self.mem_every = config['mem_every']
        self.deep_update_every = config['deep_update_every']
    ...
    def step(self, ce_image, bm_image, mask=None, valid_labels=None, end=False):
        # image: 3*H*W
        # mask: num_objects*H*W or None
        self.curr_ti += 1
        ce_image, self.pad = pad_divide_by(ce_image, 16)
        bm_image, self.pad = pad_divide_by(bm_image, 16)
        ce_image = ce_image.unsqueeze(0) # add the batch dimension
        bm_image = bm_image.unsqueeze(0) # add the batch dimension
        ...
        key, shrinkage, selection, f16, f8, f4 = self.network.encode_key(ce_image, bm_image, 
                                                    need_ek=(self.enable_long_term or need_segment), 
                                                    need_sk=is_mem_frame)
        multi_scale_features = (f16, f8, f4)
        ...
        # save as memory if needed
        if is_mem_frame:
            value, hidden = self.network.encode_value(ce_image, bm_image, f16, self.memory.get_hidden(), 
                                    pred_prob_with_bg[1:].unsqueeze(0), is_deep_update=is_deep_update)
            self.memory.add_memory(key, shrinkage, value, self.all_labels, 
                                    selection=selection if self.enable_long_term else None)
            self.last_mem_ti = self.curr_ti
