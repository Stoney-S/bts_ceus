...
from inference.data.video_reader import VideoReader, VideoReader_multi_
...
class CEUSVOSTestDataset:
    def __init__(self, ce_image_dir, bm_image_dir, mask_dir, size=-1):
        self.ce_image_dir = ce_image_dir
        self.bm_image_dir = bm_image_dir
        self.mask_dir = mask_dir
        self.size = size

        self.vid_list = sorted(os.listdir(self.ce_image_dir))

    def get_datasets(self):
        for video in self.vid_list:
            yield VideoReader_multi_(video, 
                path.join(self.ce_image_dir, video),
                path.join(self.bm_image_dir, video),
                path.join(self.mask_dir, video),
                to_save = [
                    name[:-4] for name in os.listdir(path.join(self.mask_dir, video))
                ],
